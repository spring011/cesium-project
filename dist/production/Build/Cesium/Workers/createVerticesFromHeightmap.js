define([
  ...
  Je.computeVertices = function (e) {
    ...
    var v = Ye.defaultValue(e.exaggeration, 1),
      ...
      A = Ye.defaultValue(T.elementsPerHeight, Je.DEFAULT_STRUCTURE.elementsPerHeight),
      B = Ye.defaultValue(T.stride, Je.DEFAULT_STRUCTURE.stride),
      D = Ye.defaultValue(T.elementMultiplier, Je.DEFAULT_STRUCTURE.elementMultiplier),
    ...
    U &&
      ...
      (O = 1 / (je.WebMercatorProjection.geodeticLatitudeToMercatorAngle(r) - L)));
    var Z = et;
    ...
    for (var ne = te; ne < ae; ++ne) {
      ...
      var ce,
        ...
        he = s(le),
        me = _ * he;
      U && (ce = (je.WebMercatorProjection.geodeticLatitudeToMercatorAngle(le) - L) * O);
      for (var ge = ie; ge < re; ++ge) {
        ...
        var we,
          ...
          ke = se * (h * B) + pe * B;
        if (1 === A) we = d[ke];
        else if (((we = 0), S)) for (xe = 0; xe < A; ++xe) we = we * D + d[ke + xe];
    ...
    Ye.defined(y) && (Fe = qe.OrientedBoundingBox.fromRectangle(y, H, Y, w)),
      b &&
        (Le = new Qe.EllipsoidalOccluder(w).computeHorizonCullingPointPossiblyUnderEllipsoid(
          I,
          J,
          H,
        ));
  ...
  (f = {
    ...
    decode: function (e, t) {
      var a = (t = t || {}).encodedMaskData || null === t.encodedMaskData,
        ...
        n = u(i, t.pixelType || Float32Array, t.encodedMaskData, r, t.returnMask),
        s = {
          width: i.width,
          height: i.height,
      ...
      return (
        ...
        t.returnFileInfo &&
          ((s.fileInfo = c(i)), t.computeUsedBitDepths && (s.fileInfo.bitDepths = d(i))),
        s
      );
    },
  }),
          ...
          for (var y = 0; y <= f; y++) {
            ...
            if (0 !== I) {
              ...
              if (
                ...
                for (p = 0; p < k; p++) {
                  for (7 & M && ((T = a[M >> 3]), (T <<= 7 & M)), g = 0; g < I; g++)
                    7 & M || (T = a[M >> 3]),
                      128 & T
                        ? (l && (l[M] = 1),
                ...
                for (p = 0; p < k; p++) {
                  for (g = 0; g < I; g++) (m = (n = b[v++]) < m ? n : m), (s[M++] = n);
                  M += V;
                }
              else
    ...
    (B = function (e, t, a, i, r, n, s) {
      ...
      for (e[e.length - 1] <<= 8 * m, l = 0; l < a; l++) {
        if ((0 === d && ((f = e[c++]), (d = 32)), t <= d)) (o = (f >>> (d - t)) & u), (d -= t);
        else {
          var g = t - d;
    ...
    (R = {
      ...
      readDataOneSweep: function (e, t, a) {
        ...
        if (i.length === l * s) t.pixels.resultPixels = i;
          ...
          if (1 < s)
            for (m = 0; m < s; m++)
              for (g = m * l, h = 0; h < l; h++) u[h] && (t.pixels.resultPixels[g + h] = i[d++]);
          else for (h = 0; h < l; h++) u[h] && (t.pixels.resultPixels[h] = i[d++]);
        }
      ...
      readHuffmanTree: function (e, t) {
        ...
        var y = 0,
          I = 0,
          b = new C();
        for (o = 0; o < d.length; o++) void 0 !== d[o] && (y = Math.max(y, d[o].first));
        ...
        for (o = n; o < s; o++)
          if (0 < (c = d[(f = o - (o < r ? 0 : r))].first))
            if (((v = [c, f]), c <= I))
              for (U = d[f].second << (I - c), T = 1 << (I - c), u = 0; u < T; u++) A[U | u] = v;
            else
            ...
            for (o = u = 0; o < m; o++)
              for (f = 0; f < g; f++, u++) {
                ...
                32 <= b && ((b -= 32), (B = y[++I])),
                  (n = r - T),
                  D
                    ? ((n += !(0 < f) && 0 < o ? P[u - g] : V), (n &= 255), (V = P[u] = n))
                    : (P[u] = n);
              }
          else
            for (o = u = 0; o < m; o++)
              for (f = 0; f < g; f++, u++)
                if (M[u]) {
                  ...
                  32 <= b && ((b -= 32), (B = y[++I])),
                    (n = r - T),
                    D
                      ? (!(0 < f && M[u - 1]) && 0 < o && M[u - g] ? (n += P[u - g]) : (n += V),
                        (n &= 255),
                        (V = P[u] = n))
                      : (P[u] = n);
                }
          ...
          for (v = I !== u - 1 ? s : F, b = 0; b < f; b++)
            for (A = I * r * s + b * s, B = r - (U = b !== f - 1 ? s : L), x = 0; x < O; x++) {
              if (
                ...
                (E = 0),
                E++,
                (V = ((M = (c = new DataView(e, t.ptr, Math.min(10, T))).getUint8(0)) >> 6) & 255),
                ((M >> 2) & 15) !== (((b * s) >> 3) & 15))
              )
                throw 'integrity issue';
              if (3 < (g = 3 & M)) throw ((t.ptr += E), 'Invalid block encoding (' + g + ')');
              if (2 != g)
                if (0 == g) {
                  if (
                ...
                else if (((t.ptr += E), R.decodeBits(e, t, C, w, x), (E = 0), N))
                  for (k = 0; k < v; k++) {
    ...
    (k = {
      decode: function (e, t) {
        var a = (t = t || {}).noDataValue,
          i = 0,
          r = {};
        if (((r.ptr = t.inputOffset || 0), (r.pixels = {}), R.readHeaderInfo(e, r))) {
          var n = r.headerInfo,
            s = n.fileVersion,
