define([
...
], function (t, ut, e, lt, ft, ct, dt, Ct, pt, yt, _t, d) {
  'use strict';
  var vt = new ft.Cartesian3(),
    ht = new ft.Cartesian3(),
    At = new ft.Cartesian3(),
    xt = new ft.Cartesian3(),
    bt = new ft.Cartesian3(),
    l = new ft.Cartesian3(1, 1, 1),
    kt = Math.cos,
    wt = Math.sin;
  function C(t) {
    ...
    (this._radii = ft.Cartesian3.clone(e)),
      (this._innerRadii = ft.Cartesian3.clone(a)),
  ...
  (C.packedLength = 2 * ft.Cartesian3.packedLength + d.VertexFormat.packedLength + 7),
    (C.pack = function (t, e, a) {
      return (
        (a = ut.defaultValue(a, 0)),
        ft.Cartesian3.pack(t._radii, e, a),
        (a += ft.Cartesian3.packedLength),
        ft.Cartesian3.pack(t._innerRadii, e, a),
        (a += ft.Cartesian3.packedLength),
  ...
  (C.unpack = function (t, e, a) {
    ...
    var i = ft.Cartesian3.unpack(t, e, p);
    e += ft.Cartesian3.packedLength;
    var r = ft.Cartesian3.unpack(t, e, y);
    e += ft.Cartesian3.packedLength;
    ...
    var o = t[e++],
      ...
      c = t[e];
    return ut.defined(a)
      ? ((a._radii = ft.Cartesian3.clone(i, a._radii)),
        (a._innerRadii = ft.Cartesian3.clone(r, a._innerRadii)),
        ...
        (a._slicePartitions = f),
        (a._offsetAttribute = -1 === c ? void 0 : c),
        a)
      : ((v.minimumClock = o),
        (v.maximumClock = m),
        (v.minimumCone = s),
        ...
        (v.stackPartitions = l),
        (v.slicePartitions = f),
        (v.offsetAttribute = -1 === c ? void 0 : c),
        new C(v));
  }),
    (C.createGeometry = function (t) {
      ...
      if (!(e.x <= 0 || e.y <= 0 || e.z <= 0)) {
        ...
        if (!(a.x <= 0 || a.y <= 0 || a.z <= 0)) {
          ...
          (l = Math.round((l * Math.abs(o - n)) / lt.CesiumMath.TWO_PI)) < 2 && (l = 2),
            (f = Math.round((f * Math.abs(s - m)) / lt.CesiumMath.PI)) < 2 && (f = 2);
          var c = 0,
            d = [m],
            C = [n];
          for (i = 0; i < f; i++) d.push(m + (i * (s - m)) / (f - 1));
          for (d.push(s), r = 0; r < l; r++) C.push(n + (r * (o - n)) / (l - 1));
          C.push(o);
          var p = d.length,
            y = C.length,
            _ = 0,
          ...
          h &&
            ...
            0 < m && ((A = !0), (_ += l - 1)),
            s < Math.PI && ((x = !0), (_ += l - 1)),
            (o - n) % lt.CesiumMath.TWO_PI ? ((b = !0), (_ += 2 * (f - 1) + 1)) : (_ += 1));
          var k = y * p * v,
          ...
          for (i = 0; i < p; i++)
            for (r = 0; r < y; r++)
              (w[c++] = e.x * O[i] * z[r]), (w[c++] = e.y * O[i] * E[r]), (w[c++] = e.z * I[i]);
          var N,
            R,
            U,
            S,
          ...
          var J,
            ...
            q = 0,
            H = k / 2,
            K = ft.Ellipsoid.fromCartesian3(e),
            Q = ft.Ellipsoid.fromCartesian3(a);
          if (u.st || u.normal || u.tangent || u.bitangent) {
            for (i = 0; i < k; i++) {
              J = F[i] ? Q : K;
              var $ = ft.Cartesian3.fromArray(w, 3 * i, vt),
                tt = J.geodeticSurfaceNormal($, ht);
              if ((P[i] && ft.Cartesian3.negate(tt, tt), u.st)) {
                var et = ft.Cartesian2.negate(tt, bt);
                (L[X++] = Math.atan2(et.y, et.x) / lt.CesiumMath.TWO_PI + 0.5),
                  (L[X++] = Math.asin(tt.z) / Math.PI + 0.5);
              }
              if (
                (u.normal && ((T[Z++] = tt.x), (T[Z++] = tt.y), (T[Z++] = tt.z)),
                ...
                if (
                  (F[i] && (rt = H),
                  (at =
                    !A && rt <= i && i < rt + 2 * y ? ft.Cartesian3.UNIT_X : ft.Cartesian3.UNIT_Z),
                  ft.Cartesian3.cross(at, tt, it),
                  ft.Cartesian3.normalize(it, it),
                  u.tangent && ((D[j++] = it.x), (D[j++] = it.y), (D[j++] = it.z)),
                  u.bitangent)
                ) {
                  var nt = ft.Cartesian3.cross(tt, it, xt);
                  ft.Cartesian3.normalize(nt, nt),
                    (G[q++] = nt.x),
                    (G[q++] = nt.y),
                    (G[q++] = nt.z);
                }
          ...
          if (ut.defined(t._offsetAttribute)) {
            var ot = w.length,
              ...
              st = t._offsetAttribute === _t.GeometryOffsetAttribute.NONE ? 0 : 1;
            _t.arrayFill(mt, st),
          ...
          return new Ct.Geometry({
            ...
            boundingSphere: ct.BoundingSphere.fromEllipsoid(K),
