define([
  ...
  './Plane-2d882f9f',
  './WebMercatorProjection-44bf888f',
  './createTaskProcessorWorker',
  './EllipsoidTangentPlane-30395e74',
  './OrientedBoundingBox-e6450288',
  './TerrainEncoding-32e7a288',
], function (We, e, Fe, Oe, Ye, ke, t, i, n, r, a, Ue, o, Ve, He, Le) {
  'use strict';
  var De = Uint16Array.BYTES_PER_ELEMENT,
    Ge = Int32Array.BYTES_PER_ELEMENT,
    je = Uint32Array.BYTES_PER_ELEMENT,
    ze = Float32Array.BYTES_PER_ELEMENT,
    qe = Float64Array.BYTES_PER_ELEMENT;
  function Je(e, t, i) {
    i = We.defaultValue(i, Fe.CesiumMath);
    for (var n = e.length, r = 0; r < n; ++r)
      if (i.equalsEpsilon(e[r], t, Fe.CesiumMath.EPSILON12)) return r;
    return -1;
  }
  var Ke = new Oe.Cartographic(),
    Qe = new Oe.Cartesian3(),
    Xe = new Oe.Cartesian3(),
  ...
  function et(e, t, i, n, r, a, o, s, u, h) {
    for (var d = o.length, c = 0; c < d; ++c) {
      ...
      (a.hMin = Math.min(a.hMin, f)),
        Oe.Cartographic.fromRadians(v, I, f, Ke),
        u && (Ke.longitude += s),
        u ? (c === d - 1 ? (Ke.latitude += h) : 0 === c && (Ke.latitude -= h)) : (Ke.latitude += s);
      var E = a.ellipsoid.cartographicToCartesian(Ke);
      e.push(E),
        t.push(f),
        i.push(Oe.Cartesian2.clone(i[m])),
        0 < n.length && n.push(n[m]),
        Ye.Matrix4.multiplyByPoint(a.toENU, E, Qe);
      var T = a.minimum,
        C = a.maximum;
  ...
  return o(function (e, t) {
    (e.ellipsoid = Oe.Ellipsoid.clone(e.ellipsoid)),
      (e.rectangle = Oe.Rectangle.clone(e.rectangle));
    var i = (function (e, t, i, n, r, a, o, s, u, h) {
        var d, c, g, l, m, p;
        p = We.defined(n)
          ? ((d = n.west), (c = n.south), (g = n.east), (l = n.north), (m = n.width), n.height)
          : ((d = Fe.CesiumMath.toRadians(r.west)),
            (c = Fe.CesiumMath.toRadians(r.south)),
            (g = Fe.CesiumMath.toRadians(r.east)),
            (l = Fe.CesiumMath.toRadians(r.north)),
            (m = Fe.CesiumMath.toRadians(n.width)),
            Fe.CesiumMath.toRadians(n.height));
        var v,
          I,
          f = [c, l],
          E = [d, g],
          T = Ye.Transforms.eastNorthUpToFixedFrame(t, i),
          C = Ye.Matrix4.inverseTransformation(T, $e);
        s &&
          ((v = Ue.WebMercatorProjection.geodeticLatitudeToMercatorAngle(c)),
          (I = 1 / (Ue.WebMercatorProjection.geodeticLatitudeToMercatorAngle(l) - v)));
        var M = new DataView(e),
        ...
        for (P = B = 0; P < 4; ++P) {
          w = M.getUint32(B, !0);
          var K = (B += je),
            Q = Fe.CesiumMath.toRadians(180 * M.getFloat64(B, !0));
          B += qe;
          var X = Fe.CesiumMath.toRadians(180 * M.getFloat64(B, !0));
          B += qe;
          var Z = Fe.CesiumMath.toRadians(180 * M.getFloat64(B, !0)),
            $ = 0.5 * Z;
          B += qe;
          var ee = Fe.CesiumMath.toRadians(180 * M.getFloat64(B, !0)),
            te = 0.5 * ee;
          B += qe;
          ...
          for (var re = new Array(ie), ae = 0; ae < ie; ++ae) {
            ...
            (re[ae] = q),
              Math.abs(oe - d) < $
                ? D.push({ index: q, cartographic: Oe.Cartographic.clone(Ke) })
                : Math.abs(oe - g) < $
                  ? j.push({ index: q, cartographic: Oe.Cartographic.clone(Ke) })
                  : Math.abs(se - c) < te
                    ? G.push({ index: q, cartographic: Oe.Cartographic.clone(Ke) })
                    : Math.abs(se - l) < te &&
                      z.push({ index: q, cartographic: Oe.Cartographic.clone(Ke) }),
              (N = Math.min(ue, N)),
              (x = Math.max(ue, x)),
              (V[q] = ue);
            var de = i.cartographicToCartesian(Ke);
            (k[q] = de),
              s && (H[q] = (Ue.WebMercatorProjection.geodeticLatitudeToMercatorAngle(se) - v) * I),
              Ye.Matrix4.multiplyByPoint(C, de, Qe),
              Oe.Cartesian3.minimumByComponent(Qe, b, b),
              Oe.Cartesian3.maximumByComponent(Qe, S, S);
            var ce = (oe - d) / (g - d);
            ce = Fe.CesiumMath.clamp(ce, 0, 1);
            var ge = (se - c) / (l - c);
            (ge = Fe.CesiumMath.clamp(ge, 0, 1)), (U[q] = new Oe.Cartesian2(ce, ge)), ++q;
          }
          for (var le = 3 * ne, me = 0; me < le; ++me, ++J)
            (L[J] = re[M.getUint16(B, !0)]), (B += De);
        ...
        var fe = 1e-5;
        if (
          (et(k, V, U, H, L, Ie, D, -fe * m, !0, -fe * p),
          et(k, V, U, H, L, Ie, G, -fe * p, !1),
          et(k, V, U, H, L, Ie, j, fe * m, !0, fe * p),
          et(k, V, U, H, L, Ie, z, fe * p, !1),
          0 < D.length && 0 < z.length)
        ) {
          var Ee = D[0].index,
            Te = z[z.length - 1].index,
            Ce = k.length - 1;
          L.push(Te, Ce, pe, pe, Ee, Te);
        }
        y = k.length;
        var Me,
          Ne = Ye.BoundingSphere.fromPoints(k);
        We.defined(n) && (Me = He.OrientedBoundingBox.fromRectangle(n, N, x, i));
        for (
          var xe = new Le.EllipsoidalOccluder(i).computeHorizonCullingPointPossiblyUnderEllipsoid(
              t,
              k,
              N,
            ),
            be = new Ve.AxisAlignedBoundingBox(b, S, t),
            Se = new Le.TerrainEncoding(be, Ie.hMin, x, T, !1, s),
            we = new Float32Array(y * Se.getStride()),
            Pe = 0,
            Be = 0;
          Be < y;
          ++Be
        )
          Pe = Se.encode(we, Pe, k[Be], U[Be], V[Be], void 0, H[Be]);
        var ye = D.map(function (e) {
          ...
          _e = z
            ...
            .reverse();
        return (
          Ae.unshift(Re[Re.length - 1]),
          Ae.push(ye[0]),
          _e.unshift(ye[ye.length - 1]),
          _e.push(Re[0]),
          {
            vertices: we,
