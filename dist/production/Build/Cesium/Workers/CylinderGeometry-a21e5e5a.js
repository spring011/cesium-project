  ...
  (d.unpack = function (t, e, a) {
    ...
    return I.defined(a)
      ...
      : ((f.length = n),
        ...
        (f.bottomRadius = i),
        (f.slices = s),
        (f.offsetAttribute = -1 === m ? void 0 : m),
        new d(f));
  }),
    (d.createGeometry = function (t) {
      ...
      if (!(e <= 0 || a < 0 || r < 0 || (0 === a && 0 === r))) {
        var i,
          ...
          f = n.normal ? new Float32Array(3 * u) : void 0,
          y = n.tangent ? new Float32Array(3 * u) : void 0,
        ...
        if (c) {
          ...
          for (i = 0; i < o; i++) {
            ...
            c &&
              ((x.x = G),
              (x.y = D),
              b && (C = S.Cartesian3.normalize(S.Cartesian3.cross(S.Cartesian3.UNIT_Z, x, C), C)),
              n.normal &&
                ((f[v++] = x.x),
                (f[v++] = x.y),
                (f[v++] = x.z),
          ...
          for (i = 0; i < o; i++)
            n.normal && ((f[v++] = 0), (f[v++] = 0), (f[v++] = -1)),
              n.tangent && ((y[A++] = 1), (y[A++] = 0), (y[A++] = 0)),
        ...
        for (i = 0; i < o - 1; i++)
          (V[T++] = O),
            ...
            (O += 2);
        for (
          V[T++] = s - 2, V[T++] = 0, V[T++] = 1, V[T++] = s - 2, V[T++] = 1, V[T++] = s - 1, i = 1;
          i < o - 1;
          i++
        )
          (V[T++] = s + i + 1), (V[T++] = s + i), (V[T++] = s);
        for (i = 1; i < o - 1; i++) (V[T++] = m), (V[T++] = m + i), (V[T++] = m + i + 1);
        var L = 0;
        if (n.st) {
          var P = Math.max(a, r);
          for (i = 0; i < u; i++) {
        ...
        n.position &&
          ...
          (H.x = 0.5 * e),
          (H.y = Math.max(r, a));
        var z = new B.BoundingSphere(S.Cartesian3.ZERO, S.Cartesian2.magnitude(H));
